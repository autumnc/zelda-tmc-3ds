...
target("tmc_pc")
    ...
    add_files("port/port_vaati_progress.c")  -- Vaati finale progression repair (vaatiRebornEnemy)
    add_files("port/port_bottle_compat.c")   -- bottle-reward chest eligibility (playerItemUtils)
    add_files("port/port_3ds_full_view_policy.c") -- 3DS Full View policy matrix (linked from port_linked_stubs.c)
    add_files("port/port_cloud_tops_fight.c") -- Cloud Tops finale EU fight repair (fileselect/roomInit)
...
-- ====================
-- Transition preserved-coordinate regression test.
-- ====================
target("transition_axis_test")
    set_kind("binary")
    set_languages("c11")
    set_targetdir("build/pc")
    add_includedirs(".")
    add_includedirs("port")
    add_includedirs("include")
    add_files("port/port_transition_axis_test.c")
target_end()
